...
%token <keyword> ABORT_P ABSENT ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	CLUSTER COALESCE COLLATE COLLATION COLLECTION COLUMN COLUMNS COMMENT COMMENTS COMMIT
...
CreateStmt:	CREATE OptTemp COLLECTION qualified_name
				{
					CreateStmt *n = makeNode(CreateStmt);
					ColumnDef *id_col = makeNode(ColumnDef);
					ColumnDef *doc_col = makeNode(ColumnDef);
					...
					id_col->colname = "_id";
					id_col->typeName = SystemTypeName("text");
					id_col->is_local = true;
					id_col->is_not_null = true;
					id_col->storage = 0;
					id_col->raw_default = NULL;
					id_col->cooked_default = NULL;
					id_col->collOid = InvalidOid;

					pk_constr->contype = CONSTR_PRIMARY;
					pk_constr->location = @3;
					id_col->constraints = list_make1(pk_constr);

					doc_col->colname = "document";
					doc_col->typeName = SystemTypeName("jsonb");
					doc_col->is_local = true;
					doc_col->is_not_null = false;
					doc_col->storage = 0;
					doc_col->raw_default = NULL;
					doc_col->cooked_default = NULL;
					doc_col->collOid = InvalidOid;

					n->tableElts = list_make2(id_col, doc_col);
					n->inhRelations = NIL;
					n->partspec = NULL;
					n->ofTypename = NULL;
					n->constraints = NIL;
		...
		| CREATE OptTemp COLLECTION IF_P NOT EXISTS qualified_name
				{
					CreateStmt *n = makeNode(CreateStmt);
					ColumnDef *id_col = makeNode(ColumnDef);
					ColumnDef *doc_col = makeNode(ColumnDef);
